...
exports.traverseTree = catchAsync(async (req, res, next) => {
	...
	let hubDeptList = [];
	const treeStr = await dept.traverseTree(
		0,
		`Tree of Department ${dept.name}-${dept.username}`,
		hubTreeObj,
		hubDeptList
	);
			...
			if (dept.parent !== null) {
				...
				for (let index = 0; membersToAdd.length > index; index++) {
					if (!parentMemberGroup.members.includes(membersToAdd[index])) {
						return next(
							new AppError(
								`Provided User (id:${membersToAdd[index]}) is not a member of ParentDept. Please provide valid userId.`,
								401
							)
						);
					}
...
exports.removeMembers = catchAsync(async (req, res, next) => {
	...
	if (members && members.length > 0) {
		...
		if (dept.library) {
			...
			if (!library) {
				return next(
					new AppError(`Dept's libray id exist but Library does not exist. Something went wrong.`, 500)
				);
			}
			isLibraryExist = true;
		}
		...
		for (let i = 0; i < members.length; i++) {
			...
			if (deptControllers.includes(String(members[i]))) {
				return next(
					new AppError(
						`Members can not be removed because this (${members[i]}) exists in Dept's 'controllers' section. Remove member from 'controllers' first!`,
						401
					)
				);
			}
			...
			if (isLibraryExist && libraryControllers.includes(String(members[i]))) {
				return next(
					new AppError(
						`Members can not be removed because this (${members[i]}) exists in Library's 'controllers' section. Remove member from 'controllers' first!`,
						401
					)
				);
			}
...
exports.addControllers = catchAsync(async (req, res, next) => {
	...
	if (controllers && controllers.length > 0) {
		...
		if (controllersToAdd.length > 0) {
			...
			return next(
				new AppError(
					`Controllers can not be added because this/these controllers not member of MemberGroup. Add them to MemberGroup first!`,
					401
				)
			);
		}
			...
			for (let dc_index = 0; dept.controllers.length > dc_index; dc_index++) {
				if (String(controllers[rc_index]) === String(dept.controllers[dc_index].user)) {
					...
					if (String(dept.controllers[dc_index].role) === 'owner') {
						return next(
							new AppError(
								`Owner can not be removed. Update the ownership first at controllers section.`,
								401
							)
						);
					}
