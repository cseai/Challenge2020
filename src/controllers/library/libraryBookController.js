...
exports.getAllBooks = catchAsync(async (req, res, next) => {
	...
	const features = new APIFeatures(Book.find({ library: req.params.libraryId }), req.query)
		.filter()
		.sort()
		.limitFields()
		.paginate();
...
exports.updateBook = catchAsync(async (req, res, next) => {
	...
	if (!isReqUserController) {
		return next(
			new AppError(`Requested user must be a controller to update Book Information. Permission denied`, 401)
		);
	}
