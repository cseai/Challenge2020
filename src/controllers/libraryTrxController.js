...
exports.getAllResourceTrxs = catchAsync(async (req, res, next) => {
    // NOTE: This is ResourceTrxs of a specific Library
	
    // Filter resource trx in a specific library
	const features = new APIFeatures(Trx.find({ library: req.params.libraryId, contentType: 'resource' }), req.query).filter().sort().limitFields().paginate();
	const trxs = await features.query;
...
exports.getResourceTrx = catchAsync(async (req, res, next) => {
	// NOTE: This is ResourceTrxs of a specific Library
	
    // Filter resource trx in a specific library
	const trxQuery = Trx.findOne({library: req.params.libraryId, contentType: 'resource', _id: req.params.trxId});
    const trx = await trxQuery;
	if (!trx) {
		return next(new AppError(`Trx doesn't exists!`, 404));
    }
...
exports.borrowResourceTrx = catchAsync(async (req, res, next) => {
	// Copy data and pre-validate
	const clearedData = {...req.body};
	if(!clearedData.objectId){
		return next(new AppError(`'objectId' (resourceId) field is required`, 401));
	}
	...
	// Get Resource of this library which is `available`
	const resource = await Resource.findOne(
            { library: req.params.libraryId, _id: clearedData.objectId, status: 'available' }
        );
	if (!resource) {
		return next(new AppError(`Resource does not exist or unavailable.`, 404));
	}

	// validate data
	clearedData.library = library._id;
	clearedData.contentType = 'resource';
	clearedData.objectId = resource._id;
	// `issueDate` : Note: This should be same as created at... default: Date.now()
	clearedData.issueDate = Date.now();
	if(!clearedData.dueDate){
		// Note: this should be given by client...and validation ERROR
	...
	if(clearedData.returnDate){
		// `returnDate`: Return date will be asigned during resource return
		clearedData.returnDate = null;
	}
	if(!clearedData.user){
		// `user`: User who borrowing the resource required
		return next(new AppError(`Borrower User field is required`, 401));
	}
	else{
		// Verify the user is a member of this Library
		...
		// Verify the user is not same as requested user
		if(String(req.user._id) === String(clearedData.user)){
			return next(new AppError(`User can not borrow Resource himself/herself.`, 401));
		}
	}
	// `issuedBy`: User who created this Trx
	clearedData.issuedBy = req.user._id;
	if(clearedData.updatedBy){
		// `updatedBy` [Array]: This field will be asigned during resource Trx update
		clearedData.updatedBy = [];
	}
	if(clearedData.active){
		// `active`: This will be 'true' when trx is active... and when closed then 'false'
...
exports.returnResourceTrx = catchAsync(async (req, res, next) => {
	// Copy data and pre-validate
	const clearedData = {...req.body};
	if(!clearedData.objectId){
		return next(new AppError(`'objectId' (resourceId) field is required`, 401));
	...
	// Get Resource of this library which is `unavailable`
	const resource = await Resource.findOne(
            { library: req.params.libraryId, _id: clearedData.objectId, status: 'unavailable' }
        );
	if (!resource) {
		return next(new AppError(`Resource does not exist or already returned.`, 404));
	}
	...
	// IMPORTANT: Save changed by deactivate trx and also save resource by changing status
	await trx.save();
	// IMPORTANT: This must be done better way...like pre or post save
